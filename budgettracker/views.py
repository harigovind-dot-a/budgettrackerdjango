...
def budget_status_view(request):
    ...
    income_total = Transaction.objects.filter(user=user, category__type='income', date__month=month, date__year=year).aggregate(Sum('amount'))['amount__sum'] or 0.00
    expense_total = Transaction.objects.filter(user=user, category__type='expense', date__month=month, date__year=year).aggregate(Sum('amount'))['amount__sum'] or 0.00
